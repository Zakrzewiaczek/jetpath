...
TEST_F(CommandManagerTest, FileInfo_HasReasonableModificationDates) {
    ...
    auto one_year_ago = now - (365 * 24 * 60 * 60); 
...
TEST(testCommandManager,checkingAllAtOnce){
    addFile("/testAssets","file100.txt");
    auto files = CommandManager::getAllFiles();
    for (const auto& file : files) {
        if (file.name == "file100.txt") {
            EXPECT_EQ(file.name, "file100.txt");
            EXPECT_EQ(file.path, "/testAssets/file100.txt");
            EXPECT_TRUE(file.is_directory == false);
            EXPECT_GT(file.size, 0);
            std::cout<<file.owner<<std::endl;
            std::cout<<file.permissions<<std::endl;
            std::cout<<file.extension<<std::endl;
            EXPECT_FALSE(file.owner.empty());
            EXPECT_FALSE(file.permissions.empty());
            EXPECT_FALSE(file.extension.empty());
            EXPECT_EQ(file.owner, "stas");
            EXPECT_EQ(file.permissions, "root");
